...
DEFAULT_PRESETS: list[dict[int, dict[str, str | bool]]] = [
    {
        ...
        "name": "default",
        "default": True,
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --download-archive %(config_path)s/archive.log",
    ...
    },
    {
        ...
        "name": "1080p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --download-archive %(config_path)s/archive.log\n-S vcodec:h264 --format 'bv*[height<=1080][ext=mp4]+ba[ext=m4a]/bv*[height<=1080]+ba/b[height<=1080] / best'",
        "default": True,
    ...
    },
    {
        ...
        "name": "720p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --download-archive %(config_path)s/archive.log\n-S vcodec:h264 --format 'bv*[height<=720][ext=mp4]+ba[ext=m4a]/bv*[height<=720]+ba/b[height<=720] / best'",
        "default": True,
    ...
    {
        "id": "e0b5c1a7-9f4d-4e2b-8b5a-1c7d3f9e6a0d",
        "name": "Best (AV1)",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --download-archive %(config_path)s/archive.log\n-S vcodec:av01 --format 'bv+ba/b'",
        "default": True,
        "description": "Prefer AV1 video codec (Best Resolution).",
    },
    {
        "id": "a1b2c3d4-e5f6-4a5b-8c7d-9e0f1a2b3c4d",
        "name": "AV1 1080p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --download-archive %(config_path)s/archive.log\n-S vcodec:av01 --format 'bv*[height<=1080][ext=mp4]+ba[ext=m4a]/bv*[height<=1080]+ba/b[height<=1080]'",
        "default": True,
        "description": "Prefer AV1 video codec at 1080p.",
    },
    {
        "id": "b2c3d4e5-f6a7-5b6c-9d8e-0f1a2b3c4d5e",
        "name": "AV1 720p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --download-archive %(config_path)s/archive.log\n-S vcodec:av01 --format 'bv*[height<=720][ext=mp4]+ba[ext=m4a]/bv*[height<=720]+ba/b[height<=720]'",
        "default": True,
        "description": "Prefer AV1 video codec at 720p.",
    },
    {
        "id": "c3d4e5f6-a7b8-6c7d-8e9f-1a2b3c4d5e6f",
        "name": "AV1 480p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --download-archive %(config_path)s/archive.log\n-S vcodec:av01 --format 'bv*[height<=480][ext=mp4]+ba[ext=m4a]/bv*[height<=480]+ba/b[height<=480]'",
        "default": True,
        "description": "Prefer AV1 video codec at 480p.",
    },
    {
        "id": "d4e5f6a7-b8c9-7d8e-9f0a-2b3c4d5e6f7a",
        "name": "AV1 360p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --download-archive %(config_path)s/archive.log\n-S vcodec:av01 --format 'bv*[height<=360][ext=mp4]+ba[ext=m4a]/bv*[height<=360]+ba/b[height<=360]'",
        "default": True,
        "description": "Prefer AV1 video codec at 360p.",
    },
    {
        "id": "e5f6a7b8-c9d0-8e9f-0a1b-3c4d5e6f7a8b",
        "name": "AV1 240p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --download-archive %(config_path)s/archive.log\n-S vcodec:av01 --format 'bv*[height<=240][ext=mp4]+ba[ext=m4a]/bv*[height<=240]+ba/b[height<=240]'",
        "default": True,
        "description": "Prefer AV1 video codec at 240p.",
    },
    {
        "id": "f6a7b8c9-d0e1-9f0a-1b2c-4d5e6f7a8b9c",
        "name": "AV1 144p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --download-archive %(config_path)s/archive.log\n-S vcodec:av01 --format 'bv*[height<=144][ext=mp4]+ba[ext=m4a]/bv*[height<=144]+ba/b[height<=144]'",
        "default": True,
        "description": "Prefer AV1 video codec at 144p.",
    },
    {
        "id": "f1c6d2b8-0a5e-5f3c-9c6b-2d8e4a0f7b1e",
        "name": "Best (VP9)",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --download-archive %(config_path)s/archive.log\n-S vcodec:vp9 --format 'bv+ba/b'",
        "default": True,
        "description": "Prefer VP9 video codec.",
    },
    {
        "id": "g2d7e3c9-1b6f-6a4d-0d7c-3e9f5b1a8c2f",
        "name": "Best (HEVC/H265)",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --no-overwrites\n-S vcodec:h265 --format 'bv+1/bv+ba/b'",
        "default": True,
        "description": "Prefer HEVC (H.265) video codec (Best Resolution) with audio ID 1. Auto-numbers duplicates.",
        "template": "%(title)s [%(id)s]-%(epoch)s.%(ext)s",
    },
    {
        "id": "g3e8f5d1-2c8h-8c6f-2f9e-5g1b7c3d0e4h",
        "name": "HEVC 1080p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --no-overwrites\n-S vcodec:h265 --format 'bv*[height<=1080]+1/bv*[height<=1080]+ba/b[height<=1080]'",
        "default": True,
        "description": "Prefer HEVC (H.265) video codec at 1080p with audio ID 1. Auto-numbers duplicates.",
        "template": "%(title)s [%(id)s]-%(epoch)s.%(ext)s",
    },
    {
        "id": "h4f9g6e2-3d9i-9d7g-3g0f-6h2c8d4e1f5i",
        "name": "HEVC 720p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --no-overwrites\n-S vcodec:h265 --format 'bv*[height<=720]+1/bv*[height<=720]+ba/b[height<=720]'",
        "default": True,
        "description": "Prefer HEVC (H.265) video codec at 720p with audio ID 1. Auto-numbers duplicates.",
        "template": "%(title)s [%(id)s]-%(epoch)s.%(ext)s",
    },
    {
        "id": "i5g0h7f3-4e0j-0e8h-4h1g-7i3d9e5f2g6j",
        "name": "HEVC 480p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --no-overwrites\n-S vcodec:h265 --format 'bv*[height<=480]+1/bv*[height<=480]+ba/b[height<=480]'",
        "default": True,
        "description": "Prefer HEVC (H.265) video codec at 480p with audio ID 1. Auto-numbers duplicates.",
        "template": "%(title)s [%(id)s]-%(epoch)s.%(ext)s",
    },
    {
        "id": "j6h1i8g4-5f1k-1f9i-5i2h-8j4e0f6g3h7k",
        "name": "HEVC 360p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --no-overwrites\n-S vcodec:h265 --format 'bv*[height<=360]+1/bv*[height<=360]+ba/b[height<=360]'",
        "default": True,
        "description": "Prefer HEVC (H.265) video codec at 360p with audio ID 1. Auto-numbers duplicates.",
        "template": "%(title)s [%(id)s]-%(epoch)s.%(ext)s",
    },
    {
        "id": "k7i2j9h5-6g2l-2g0j-6j3i-9k5f1g7h4i8l",
        "name": "HEVC 240p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --no-overwrites\n-S vcodec:h265 --format 'bv*[height<=240]+1/bv*[height<=240]+ba/b[height<=240]'",
        "default": True,
        "description": "Prefer HEVC (H.265) video codec at 240p with audio ID 1. Auto-numbers duplicates.",
        "template": "%(title)s [%(id)s]-%(epoch)s.%(ext)s",
    },
    {
        "id": "l8j3k0i6-7h3m-3h1k-7k4j-0l6g2h8i5j9m",
        "name": "HEVC 144p",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --no-overwrites\n-S vcodec:h265 --format 'bv*[height<=144]+1/bv*[height<=144]+ba/b[height<=144]'",
        "default": True,
        "description": "Prefer HEVC (H.265) video codec at 144p with audio ID 1. Auto-numbers duplicates.",
        "template": "%(title)s [%(id)s]-%(epoch)s.%(ext)s",
    },
    {
        "id": "h3e8f4d0-2c7g-7b5e-1e8d-4f0a6b2c9d3g",
        "name": "Best (H264)",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --download-archive %(config_path)s/archive.log\n-S vcodec:h264 --format 'bv+ba/b'",
        "default": True,
        "description": "Prefer H.264 video codec.",
    },
    {
        "id": "m9k4l1j7-8i4n-4i2l-8l5k-1m7h3i9j6k0n",
        "name": "HEVC 720p (No Archive)",
        "cli": "--cookies %(config_path)s/cookies.txt --socket-timeout 30 --no-overwrites\n-S vcodec:h265 --format 'bv*[height<=720][ext=mp4]+ba[ext=m4a]/bv*[height<=720]+ba/b[height<=720]'",
        "default": True,
        "description": "Prefer HEVC (H.265) at 720p. Ignores archive - will re-download and auto-number duplicates (file.mp4, file.f123456.mp4, etc).",
    },
    {
